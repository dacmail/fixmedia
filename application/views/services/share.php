...
<body>
	<div class="popup">
		...
		<div class="share-wrap clearfix">
			...
			<a class="icon tw" target="_blank" href="https://twitter.com/intent/tweet?original_referer=<?=$url?>&related=fixmedia_org&text=Esta noticia necesita %23FIX - '<?= substr(urlencode($report->title), 0, 40) ?>...' <?=$url?><?= ($report->user->allow_mention_twitter && !empty($report->user->twitter)) ? ' enviada por @' . $report->user->twitter  : ' '; ?>&via=fixmedia_org">Compartir en Twitter</a>
